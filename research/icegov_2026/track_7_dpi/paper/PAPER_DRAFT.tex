...
\title{Community-Led Digital Public Infrastructure for Low-Trust Environments: Payments, Records, and Coordination Beyond the State}

\author{Anonymous Author(s)}
\affiliation{\institution{Anonymous for review}}

\begin{abstract}
Digital Public Infrastructure (DPI) is typically framed as a national-scale, state-led initiative (e.g., India Stack). This paper challenges that state-centrism by proposing a 'community-led DPI' model. We examine whether modular, interoperable digital building blocks—identity, payments, and data exchange—can be governed and operated by community institutions in environments where state infrastructure is weak or untrusted. We contribute a set of design principles for community DPI and explore the scalar challenges of interoperating with formal national systems while preserving community sovereignty.
\end{abstract}

\ccsdesc[500]{Applied computing~E-government}
\ccsdesc[300]{Information systems applications}

\keywords{Digital Public Infrastructure (DPI), Community Informatics, Low-Trust Environments, Decentralization, Public Digital Goods}
