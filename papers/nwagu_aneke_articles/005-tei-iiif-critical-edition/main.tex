...
\title{A TEI, IIIF, and Web Annotation Bridge for an Unencoded African Syllabary}
...
f/v split only. Within that foundation, the article studies digital editions, annotation standards, and cultural-heritage interoperability. The central
result is that a selector-layer sample can map ten records into TEI/Web Annotation leads while pixel coordinates remain outside the approved claim. The paper does not claim public source-image release,
...
This article applies that foundation to digital editions, annotation standards, and cultural-heritage interoperability. The goal is not to make the
...
research. The research question is: can source-layer Nwagu Aneke records be
mapped into TEI glyph declarations and Web Annotation selector leads without
claiming a complete IIIF critical edition? Hypothesis: if each mapped record
preserves its TEI locator, Web Annotation body, selector type, review status,
and missing pixel-coordinate boundary, then the article can support a standards
bridge while blocking unsupported claims about source-image annotation and
public release. The article's central result is bounded: a selector-layer sample
can map ten records into TEI/Web Annotation leads while pixel coordinates remain
outside the reviewed claim. That boundedness is the reason the article can enter
development review. It has a result, but it does not pretend that result is a
public release, a completed source edition, or a venue-ready submission.
...
Encoding guidance and tokenizer research add a downstream warning: if a partial
selector layer is silently promoted into a complete edition, later character,
interface, search, and token inventories inherit that promotion error
...
\label{tab:article_na_005}
\begin{tabular}{p{0.34\columnwidth}p{0.56\columnwidth}}
\toprule
Item & Status \\
\midrule
Article result & a selector-layer sample can map ten records into TEI/Web Annotation leads while pixel coordinates remain outside the approved claim \\
Experiment & TEI-IIIF selector run \\
...
\section{Results}
The result is: a selector-layer sample can map ten records into TEI/Web Annotation leads while pixel coordinates remain outside the approved claim. This result is useful because it advances one research
...
status is preserved. A paper can therefore investigate digital editions, annotation standards, and cultural-heritage interoperability while keeping the
...
controlled contribution to digital editions, annotation standards, and cultural-heritage interoperability. The public-facing experiment label is the TEI-IIIF selector run, and the manuscript is development-reviewed only to the extent that it preserves the result
...
tests the selector bridge as missing-work infrastructure rather than as a
complete edition. The first pass is TEI locator review. A reviewer should inspect
the ten glyph declaration leads, confirm that each locator points to the intended
row and vowel/modifier position, and decide whether the locator syntax is stable
enough for public description. If a locator is merely convenient internal
notation, the article should keep it as a development artifact rather than a
published critical-edition identifier.

The second pass is Web Annotation review. Each annotation should preserve its
body value, TextQuoteSelector target, certainty value, review status, and
mapping status. The reviewer should reject any export that drops
\texttt{human\_review\_needed} or \texttt{partial\_selector\_no\_pixel\_coordinates}, because those
fields carry the article's central limitation. A safe annotation says: this is a
selector-layer lead for later source review. An unsafe annotation implies that
the source image region has already been reviewed and cleared.

The third pass is IIIF coordinate review. At present, the experiment records zero
pixel-coordinate selectors. That zero is not a defect to hide; it is the result
that prevents overclaiming. A public manuscript may describe what coordinate
selectors would need to provide, but it should not display source crops, canvas
regions, or image-based glyph evidence unless human source review and rights
clearance are attached. The boundary between TextQuoteSelector and
coordinate-level selector must remain visible in every table and figure caption.

The fourth pass is package and venue-fit review. A standards bridge can be
valuable for digital humanities, cultural-heritage data modeling, and script
documentation venues if it makes the missing edition work explicit. It should
not be pitched as a completed IIIF manifest, Unicode proposal, public corpus, or
source-image edition. The acceptance path is to show that partial standards
mapping can reduce future publication risk by making every absent review step
machine-readable and visible to reviewers.

The fifth pass is standards-laundering review. A reviewer should inspect one TEI
row, one Web Annotation record, and one prose summary and ask whether the
standards vocabulary is doing more work than the evidence allows. If a field
name, URI, canvas label, or package structure makes the sample look more complete
than it is, the manuscript should mark that representation as unsafe. The bridge
is valuable only if it preserves incompleteness as data.

\section{Conclusion}
This article contributes a bounded research result for digital editions,
annotation standards, and cultural-heritage interoperability. It has a result, a
