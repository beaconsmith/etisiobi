...
\title{A Source-Critical Reconstruction Ledger for the Nwagu Aneke Igbo Syllabary}
...
\begin{abstract}
This article develops a bounded source-critical reconstruction paper from the
Nwagu Aneke evidence package. The fixed foundation is source-observed 26 rows by 8
vowel/modifier columns, yielding 208 records. The 27/216 layer is derived by
f/v split only. Within that foundation, the article studies source-critical reconstruction, digital humanities, and script documentation. The central
result is that a reconstructable row/vowel source ledger exists, while full glyph-corpus claims remain out of scope. The paper does not claim public source-image release,
...
This article applies that foundation to source-critical reconstruction, digital humanities, and script documentation. The goal is not to make the
...
research. The research question is: can the current local Nwagu Aneke dossier
support a source-critical reconstruction ledger without overclaiming a full glyph-level corpus? Hypothesis: if source files, chart transcription, symbol
inventory, count reconciliation, and local cache are treated as separate evidence
objects, then the repo can reconstruct a row/vowel source ledger and uncertainty
model while keeping glyph-corpus, public-release, and speculative theory claims
blocked. The article's central result is bounded: a reconstructable row/vowel source ledger exists, while full glyph-corpus claims remain out of scope. That boundedness is
the reason the article can enter development review. It has a result, but it
...
Encoding guidance and tokenizer research add a downstream warning: if a source
ledger is unstable, then later character, interface, or token inventories inherit
that instability \cite{seiProposalTips,sennrich2016bpe,kudo2018sentencepiece,bostrom2020bytepair,rust2021tokenization}.
...
\label{tab:article_na_001}
\begin{tabular}{p{0.34\columnwidth}p{0.56\columnwidth}}
\toprule
Item & Status \\
\midrule
Article result & a reconstructable row/vowel source ledger exists, while full glyph-corpus claims remain out of scope \\
Experiment & Source-ledger reconstruction run \\
...
\section{Results}
The result is: a reconstructable row/vowel source ledger exists, while full glyph-corpus claims remain out of scope. This result is useful because it advances one research
...
used as a development-stage research paper while preserving the source and
rights boundaries needed for later public work.
...
status is preserved. A paper can therefore investigate source-critical reconstruction, digital humanities, and script documentation while keeping the
...
audit files, review record, source boundary, rights boundary, and reproducibility
packet. The third state is journal submission candidate: venue-specific
formatting, author metadata, legal review, final human approval, and public
release decisions are complete. This article currently targets the second state
while building evidence toward the third.
...
without pretending that every local result is already a venue-ready
article.
...
controlled contribution to source-critical reconstruction, digital humanities, and script documentation. The public-facing experiment label is the source-ledger reconstruction run, and the manuscript is approved only to the extent that it preserves the result
...
A journal-facing version of this article needs a source-review protocol rather
than a stronger adjective. The present manuscript can report a bounded
source-critical result, but acceptance work requires a human-readable pathway
from every public sentence back to the current dossier. The protocol begins with
source-file verification. A reviewer should confirm which local files constitute
the source dossier, which files are scans or chart captures, which files are
transcriptions, and which files are later analytical exports. That pass should
not change the argument. Its purpose is to prevent a later submission from
allowing a derivative table to stand in for the underlying source object.

The second pass is transcription verification. The review should select the row
and vowel/modifier ledger cells that support the 26 by 8 foundation and record
whether each cell is directly source-observed, transcribed from a chart, inferred
from a row label, or absent from the present dossier. The expected output is not
a polished figure. It is a disagreement matrix that names stable cells,
uncertain cells, and cells that require image-region coordinates before public
release. If a reviewer cannot trace a cell to a source region or transcription
event, the paper should retain the cell as part of the research ledger but avoid
claiming public glyph-level completeness.

The third pass is inventory classification. The f/v split must remain marked as
derived even where it helps design, teaching, search, or later computational
work. The reviewer should check every title, abstract sentence, table caption,
figure caption, and conclusion sentence for promotion errors. A promotion error
occurs when a derived count is described as a source count, when a speculative
normalization is described as an observed glyph, or when a future public-release
plan is described as a completed artifact. This pass is particularly important
because the manuscript is readable; readable prose can hide layer drift more
effectively than rough notes.

The fourth pass is public wording review. A journal submission should state that
the contribution is a source-critical reconstruction ledger and uncertainty
model, not a Unicode proposal, public corpus release, completed glyph grammar, or
community-authority decision. The target venue should therefore be chosen for
source-critical digital humanities, cultural-heritage data modeling, or
responsible research infrastructure rather than for standards ratification. The
acceptance pathway is to show that the ledger prevents overclaiming while making
future review easier. It is not to imply that the current paper settles the
artifact. That distinction is the manuscript's main defense against reviewer
concerns about novelty, evidence, and rights.

\section{Conclusion}
This article contributes a bounded research result for source-critical
reconstruction, digital humanities, and script documentation. It has a result, a
