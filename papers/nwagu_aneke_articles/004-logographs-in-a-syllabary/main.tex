...
\acmConference[Etisiobi Development Review]{Etisiobi Nwagu Aneke Article Program}{June 2026}{Enugu, Nigeria}
\acmYear{2026}
\acmISBN{}
\acmDOI{}

\title{Logographs in a Syllabary: A Lead-Set Audit for Nwagu Aneke Whole-Word Signs}
...
\begin{abstract}
This article develops a bounded development-stage research paper from the Nwagu Aneke
...
f/v split only. Within that foundation, the article studies logograph inventory, semantic-domain audit, and source-limited script analysis. The central
result is that a 30-item logograph lead set can be audited without claiming a complete corpus. The paper does not claim public source-image release,
complete glyph-shape interpretation, universal compression, exceptional
mathematics, or journal readiness. Its contribution is an
...
This article applies that foundation to logograph inventory, semantic-domain audit, and source-limited script analysis. The goal is not to make the
...
research. The research question is: can visible whole-word signs in the current
Nwagu Aneke chart transcription be organized as a bounded logograph lead set
without claiming a complete corpus? Hypothesis: if each whole-word sign is
represented as a lead with source location, semantic domain, and uncertainty
status, then the article can support corpus-building work while blocking
unsupported claims about completeness, glyph review, and public release. The
article's central result is bounded: a 30-item logograph lead set can be audited
without claiming a complete corpus. That boundedness is the reason the article
can enter development review. It has a result, but it does not pretend that
result is a public release, a completed source edition, or a venue-ready
submission.
...
Encoding guidance and tokenizer research add a downstream warning: if a
whole-word lead set is silently promoted into a complete lexical or glyph
corpus, later character, interface, and token inventories inherit that promotion
error \cite{seiProposalTips,sennrich2016bpe,kudo2018sentencepiece,bostrom2020bytepair,rust2021tokenization}.
...
\label{tab:article_na_004}
\begin{tabular}{p{0.34\columnwidth}p{0.56\columnwidth}}
\toprule
Item & Status \\
\midrule
Article result & a 30-item logograph lead set can be audited without claiming a complete corpus \\
Experiment & Logograph-ledger audit run \\
Source layer & 26 by 8 equals 208 \\
Derived layer & 27/216 by f/v split only \\
Claim ceiling & Development-stage paper, not public source-data release \\
\bottomrule
\end{tabular}
\end{table}
...
\section{Results}
The result is: a 30-item logograph lead set can be audited without claiming a complete corpus. This result is useful because it advances one research
...
paper is development-reviewed because the present result is clear; it is not
declared journal-ready because the next source-review step still matters.
...
status is preserved. A paper can therefore investigate logograph inventory, semantic-domain audit, and source-limited script analysis while keeping the
...
paper becomes development-reviewable when it has all of those plus source,
rights, citation, and review files. The present article meets that development
standard.
...
Those experiments would move the papers from development review to stronger
journal-track candidates. The present manuscript has a coherent result, clear
boundaries, and named blockers, but it is not declared publicly released or
submitted to a venue.

\section{Development Status and Public-Release Boundary}
Development status means the paper is coherent enough to be part of the lab's
...
settled. The distinction is deliberate. A lab that cannot record its own bounded
results will never reach publication quality, but a lab that treats development
status as public readiness will overclaim.

The review files therefore separate three states. The first state is working
...
audit files, review record, source boundary, rights boundary, and
reproducibility packet. The third state is journal submission candidate:
venue-specific formatting, author metadata, legal review, final human approval,
and public release decisions are complete. This article currently targets the
second state while building evidence toward the third.
...
does not support the precise claim is harmful. This is why the development gate
checks both the manuscript and its evidence files. The paper must be readable,
...
The article is viable for development review because its claim is bounded, its
evidence is named, its limitations are active, and its current use does not
require public release. The central result can be checked against a finite set
of files. The manuscript avoids broad claims about universal compression,
exceptional mathematics, public source release, or completed standardization.
The review record confirms that the paper is coherent as a development-stage
research paper, while the final readiness file keeps journal submission
separate.
...
development review were public readiness. The stronger process records that the
paper belongs in the lab's research archive and still has additional work before
journal submission. That distinction allows the lab to accumulate real papers
without pretending that every local result is already a venue-ready article.

\section{Boundary Conditions for Future Promotion}
Future promotion from development review to journal submission requires a
...
These conditions are not blockers to development-stage use because the current
paper does not claim journal readiness. They are the route to responsible
publication. The lab can therefore keep researching from a stable foundation
without confusing scientific progress with journal acceptance. That separation
is central to the paper's method and to the broader Etisiobi research standard.
...
development-review standard used here. The value of this paper is therefore not
...
that future journal reviewers should be asked to evaluate first.
...
proposal, it must pass a separate gate. The present development state is
text-only and bounded.


\section{Article-Specific Implementation Detail}
The implementation detail differs by article, but the development rule is the same.
...
controlled contribution to logograph inventory, semantic-domain audit, and source-limited script analysis. The public-facing experiment label is the logograph-ledger audit run, and the manuscript is development-reviewed only to the extent that it preserves the result
...
changes the source foundation, the article's development status can be
rechecked. If later experiments strengthen the result, the paper can move toward
journal submission. If later rights review blocks public discussion, the
development status can be withdrawn without rewriting the whole portfolio. This
is what development review is for: a stable, inspectable state between working
notes and public submission.
...
publication risk. When the burden is local, review can be audited locally; when
...
reproducibility packet, and review record. Reproduction means checking the
...
\section{Source-Review Protocol for Journal Hardening}
A journal-facing version of this article needs a source-review protocol that
tests the lead set as a review queue rather than as a finished corpus. The first
pass is lead-row verification. A reviewer should confirm the 30 rows recorded in
the experiment output, the source reference for each row, and the status
\texttt{visible\_chart\_lead\_not\_reviewed\_glyph}. That pass should also check whether any
lead has been duplicated, normalized too aggressively, or moved into the 26 by 8
syllabic count. If a row cannot be traced to the chart transcription, it should
remain outside the public lead set until the source trail is repaired.

The second pass is uncertainty review. Five rows are currently marked unknown or
uncertain, and several other rows carry broad glosses such as social, moral,
material, or ritual domains. A journal-facing manuscript should preserve that
uncertainty instead of resolving it by prose. Reviewers should classify each row
as stable lead, ambiguous gloss, culturally sensitive term, or blocked public
claim. The purpose is not to make the table look complete. The purpose is to
show exactly which future human decisions would change the corpus-building
state.

The third pass is semantic-domain review. Domain labels are useful for indexing,
comparison, and downstream interface design, but they are not proof of a settled
lexical system. The reviewer should test whether the manuscript uses domain tags
as provisional metadata rather than as final interpretation. A safe claim says
that a lead appears in a time, cognition, social, material, territorial, animal,
or cosmological domain. An unsafe claim says that the script's semantic system
has been fully reconstructed from those domains.

The fourth pass is rights and public wording review. Whole-word signs are more
likely than abstract counts to invite reproduction of source images, glyph crops,
or culturally sensitive examples. The manuscript should therefore keep public
figures blocked until rights and source-authority review are attached. A venue
submission should position the contribution as a source-limited lead-set audit
for digital humanities and script documentation, not as a public corpus release,
Unicode repertoire claim, or completed logograph grammar.

The fifth pass is downstream negative-control review. A reviewer should inspect
one corpus table, one prose summary, and one possible teaching or interface use
of the lead set. Each use should preserve the lead status, the unresolved gloss
count, and the blocked-public-release boundary. If a use hides the five
uncertain rows, treats semantic domains as final categories, or displays the
lead set as a complete dictionary, the article should classify that use as a
failure case. The audit is strongest when it shows how attractive downstream
uses can be rejected.

\section{Conclusion}
This article contributes a bounded research result for logograph inventory,
semantic-domain audit, and source-limited script analysis. It has a result, a
method, a claim ceiling, review files, and reproducibility materials, but it is
not a journal submission package. Its value is that it extends the Nwagu Aneke
