...
\title{The f/v Hinge in Nwagu Aneke: Derived Orthographic Expansion Without Source-Layer Promotion}
...
f/v split only. Within that foundation, the article studies orthographic modeling, source criticism, and design-layer governance. The central
result is that the f/v split is a derived hinge operation rather than a primary row-count claim. The paper does not claim public source-image release,
...
This article applies that foundation to orthographic modeling, source criticism, and design-layer governance. The goal is not to make the
...
research. The research question is: can the f/v split be used as an
orthographic modeling operation without promoting 27 rows or 216 cells into the
source-observed inventory? Hypothesis: if the combined source f/v row is split
only under an explicit derived-operation label, then downstream design and
computational claims can use the split while preserving the primary 26 by 8
source foundation. The article's central result is bounded: the f/v split is a
derived hinge operation rather than a primary row-count claim. That boundedness
is the reason the article can enter development review. It has a result, but it
does not pretend that result is a public release, a completed source edition, or
a venue-ready submission.
...
Encoding guidance and tokenizer research add a downstream warning: if a derived
orthographic split is silently promoted into a source inventory, later character,
interface, and token inventories inherit that promotion error
...
\label{tab:article_na_003}
\begin{tabular}{p{0.34\columnwidth}p{0.56\columnwidth}}
\toprule
Item & Status \\
\midrule
Article result & the f/v split is a derived hinge operation rather than a primary row-count claim \\
Experiment & F/v hinge audit run \\
...
\section{Results}
The result is: the f/v split is a derived hinge operation rather than a primary row-count claim. This result is useful because it advances one research
...
status is preserved. A paper can therefore investigate orthographic modeling, source criticism, and design-layer governance while keeping the
...
controlled contribution to orthographic modeling, source criticism, and design-layer governance. The public-facing experiment label is the f/v hinge audit run, and the manuscript is development-reviewed only to the extent that it preserves the result
...
tests the hinge, not a broader claim about Nwagu Aneke phonology. The first pass
is a source-row review. A reviewer should confirm whether the current dossier
supports a combined f/v source row across the eight vowel/modifier columns. The
review should name the source files, chart transcription events, and any missing
image-region coordinates. If that review changes the combined-row reading, the
experiment result must be revised before the manuscript moves forward.

The second pass is a derived-operation review. The reviewer should inspect the
transformation that converts the combined f/v row into separate f and v rows.
The output should identify the eight added cells, the conditions under which the
split may be used, and the claims that remain forbidden. A permitted use can
include teaching views, design grammars, search indexes, or normalization layers
where the split is explicitly labeled. A forbidden use includes describing 27
rows as source-observed, describing 216 cells as the primary inventory, or using
the split as evidence of completed phonological review.

The third pass is public wording review. The manuscript should be checked at the
title, abstract, table-caption, figure-caption, and conclusion levels because
those are the places where derived operations often become source claims. The
reviewer should flag any sentence that lets a reader infer that the f/v split is
a discovered source row rather than a controlled modeling operation. The safest
public wording is explicit: the source layer remains 26 by 8, and 27 by 8 is a
derived view available only after the f/v hinge operation is declared.

The fourth pass is venue-fit review. This article should be submitted only to a
venue prepared to evaluate source-critical digital humanities, cultural-heritage
data modeling, orthographic documentation, or responsible AI research
infrastructure. It should not be positioned as a Unicode proposal, a complete
script grammar, or a public corpus paper. The acceptance path is to show that a
small formal rule prevents a large class of downstream overclaims. That is a
modest contribution, but it is the honest one.

The fifth pass is downstream-use review. A reviewer should inspect at least one
table, one prose claim, and one possible software or teaching use of the split.
For each use, the reviewer should ask whether the user can still see the source
combined row, the derived split operation, and the claim ceiling. If any use
keeps only the cleaner 27-row view, the article should treat that use as a
failed application rather than as evidence of adoption. This matters because the
f/v hinge is attractive precisely where it is risky: it makes the inventory look
more regular. Journal hardening must therefore prove that regularity does not
erase provenance.

\section{Conclusion}
This article contributes a bounded research result for orthographic modeling,
source criticism, and design-layer governance. It has a result, a method, a
claim ceiling, review files, and reproducibility materials, but it is not a
journal submission package. Its value is that it extends the Nwagu Aneke
