...
\title{Comparative Evidence Conditions for African Script Standardization: Positioning Nwagu Aneke}
...
f/v split only. Within that foundation, the article studies comparative script standardization and evidence-condition analysis. The central
result is that a comparative matrix positions Nwagu Aneke as an unencoded script with review-blocked evidence conditions. The paper does not claim public source-image release,
...
This article applies that foundation to comparative script standardization and evidence-condition analysis. The goal is not to make the
...
research. The article's central result is bounded: a comparative matrix positions Nwagu Aneke as an unencoded script with review-blocked evidence conditions. That boundedness is
the reason the article can be evaluated as a field-facing comparative methods
candidate. It has a result, but it does not pretend that result is a public
release, a completed source edition, or a finished journal package.

\section{Research Question and Hypothesis}
The research question is: can a comparative African-script standardization
matrix clarify what kind of evidence condition Nwagu Aneke currently occupies
without forcing it into the same maturity category as encoded or proposal-stage
scripts? The hypothesis is that a small but source-explicit comparator set can
separate three states that are often blurred in early script documentation:
encoded scripts with stable Unicode treatment, scripts with proposal leads or
public implementation signals, and scripts whose source promise remains
review-blocked.

This question matters because comparative language can easily become a hidden
promotion mechanism. Saying that Nwagu Aneke is ``like'' Vai, Bamum, Mende
Kikakui, Kpelle, Loma, Ndebe, or Nsibidi may be useful typologically, but it is
not automatically useful evidentially. Each comparator has a different public
standardization state, and those states carry different requirements for names,
usage evidence, representative glyphs, community review, and rights. The
experiment therefore asks whether comparison can be made safer by comparing
evidence conditions rather than prestige, age, geography, or script type alone.
...
\label{tab:article_na_008}
\begin{tabular}{p{0.34\columnwidth}p{0.56\columnwidth}}
\toprule
Item & Status \\
\midrule
Article result & a comparative matrix positions Nwagu Aneke as an unencoded script with review-blocked evidence conditions \\
Experiment & Comparative standardization run \\
Source layer & 26 by 8 equals 208 \\
Derived layer & 27/216 by f/v split only \\
Claim ceiling & Bounded comparative paper, not public source-data release \\
\bottomrule
\end{tabular}
\end{table}
...
\section{Results}
The result is: a comparative matrix positions Nwagu Aneke as an unencoded script with review-blocked evidence conditions. This result is useful because it advances one research
...
The article-specific experiment compares eight African writing systems or
script proposals by evidence condition rather than by cultural importance alone.
The encoded comparator set contains Vai, Bamum, and Mende Kikakui. These are
not used as models that Nwagu Aneke already matches. They are used as mature
standardization comparators because the Unicode core specification gives them
publicly inspectable encoding status and descriptive treatment
\cite{unicodeAfricanScripts2023,seiProposalTips}. The proposal-lead and
unencoded comparator set contains Ndebe, New Nsibidi, Kpelle, Loma, and Nwagu
Aneke. This second set is heterogeneous by design: it includes scripts with
keyboard or implementation signals, scripts with proposal documents, scripts
with review activity, and the Nwagu Aneke target case where source promise is
visible but proposal readiness is blocked.

The matrix records five fields for each script: script name, type,
Unicode-status condition, standardization signal, and comparison relevance. Vai
is treated as a mature syllabary comparator. Bamum is treated as an encoded
African system with historical logographic layers. Mende Kikakui is treated as
a CV-inventory and directionality comparator. Ndebe is treated as a modern
Igbo-adjacent unencoded comparator with public implementation signals. New
Nsibidi is treated as an Igbo-adjacent public implementation and review
comparator. Kpelle and Loma are treated as West African syllabaries with
proposal histories. Nwagu Aneke is treated as the target case: source-promising,
unencoded, not on the roadmap in the recorded sources, and review-blocked.

The result is not that Nwagu Aneke belongs in the same standards maturity class
as Vai, Bamum, or Mende Kikakui. The result is the opposite. Comparison becomes
useful only after the matrix refuses maturity collapse. Nwagu Aneke can be
compared to encoded syllabaries typologically, but its standards evidence state
must remain closer to a review-blocked source dossier than to a completed
encoding record. It can be compared to Ndebe and New Nsibidi as an Igbo-adjacent
case, but even there the comparison has to distinguish public implementation
signals, proposal records, keyboard availability, and source-reviewed evidence.

The matrix also creates a negative control for future papers. If a later
manuscript cites Vai, Bamum, or Mende Kikakui in order to imply that Nwagu
Aneke is encoding-ready, it fails the control. If it cites Kpelle or Loma to
show that proposal histories exist for West African syllabaries, while still
marking Nwagu Aneke as lacking the necessary reviewed evidence package, it
passes the control. If it uses Ndebe or New Nsibidi to discuss Igbo-adjacent
script innovation but blurs implementation evidence with source authority, it
fails. The matrix thus changes comparison from rhetorical analogy into a
checkable claim structure.

This is why the experiment is a concluded result rather than a literature note.
It produces an eight-row evidence-condition artifact with a target-case
decision. The decision is that Nwagu Aneke is currently best positioned as an
unencoded, review-blocked script for comparative standardization analysis. That
position allows scholarly comparison but blocks public claims of Unicode
proposal readiness, public repertoire completion, representative glyph
approval, and community-authorized standards action. The result can be
overturned only by stronger source review, rights review, or standards evidence,
not by a more polished analogy.

The comparative method is also relevant to digital-humanities infrastructure.
Source-critical projects often need to borrow concepts from mature standards
without importing their maturity state. A TEI description, IIIF manifest, Web
Annotation record, or RO-Crate package can describe an unstable object, but the
presence of such infrastructure does not make the object stable
\cite{teiP5Glyphs,iiifPresentation3,w3cAnnotationModel,w3cAnnotationVocab,rocrate12}.
Similarly, computational representations such as tokenizers can create useful
units from sparse writing-system evidence \cite{sennrich2016bpe,kudo2018sentencepiece,bostrom2020bytepair,rust2021tokenization}, but those units do not
settle standards status. The matrix gives future systems a way to inherit
comparison without inheriting overclaim.
...
status is preserved. A paper can therefore investigate comparative script standardization and evidence-condition analysis while keeping the
...
compression, exceptional mathematics, public source release, or completed
standardization. The review record confirms that the paper is coherent as a
bounded research paper, while the readiness file keeps journal-package decisions
separate.
...
controlled contribution to comparative script standardization and evidence-condition analysis. The public-facing experiment label is the comparative standardization run, and the manuscript is evaluated only to the extent that it preserves the result
...
\section{Conclusion}
This article contributes a bounded research result for comparative script
standardization and evidence-condition analysis. It is stable enough for
